...
path = os.getcwd()
PATH_TO_DATA = path + "/large_movie_review_dataset"
    ...
    for pass_iteration in range(numpasses):
        ...
        for bow, goldlabel in examples:
            ...
            if predlabel != goldlabel:
                ...
                # print(len(features_for_label(bow, goldlabel)))
                # print(len( features_for_label(bow, predlabel)))
                # break
                g = dict_subtract(features_for_label(bow,goldlabel),features_for_label(bow,predlabel))
                # print(len(g))
                # break
...
def plot_accuracy_vs_iteration(train_acc, test_acc, avg_test_acc, naive_bayes_acc):
    ...
    naive_bayes = list()
    for i in range(1,11):
        naive_bayes.append(naive_bayes_acc)
    plt.plot(x, train_acc)
    plt.plot(x, test_acc)
    plt.plot(x, avg_test_acc)
    plt.plot(x, naive_bayes)
    plt.xlabel("Number of Iterations")
    plt.ylabel("Accuracy")
    plt.title("Comparing Classifier Accuracies")

    plt.legend(['Training - Vanilla', 'Test - Vanilla', 'Test - Averaged', 'Naive Bayes'], loc='upper left')
...
if __name__=='__main__':
    ...
    sol_dict_avg = train(training_set, do_averaging=True, devdata=test_set)
    plot_accuracy_vs_iteration(sol_dict['train_acc'], sol_dict['test_acc'], sol_dict_avg['avg_test_acc'], 0.84)
